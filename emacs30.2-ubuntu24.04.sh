...
git fetch --depth 1 origin emacs-30.2
...
# Had to switch gccjit-10 to gccjit-13 so that it matches gcc version on this machine
sudo apt install libgccjit0 libgccjit-13-dev texinfo

./autogen.sh

# Do not configure for X windows manager
./configure --with-native-compilation=aot \
            --with-tree-sitter \
            --with-modules \
            --with-threads \
            --with-x-toolkit=no \
            --with-xpm=ifavailable --with-gif=ifavailable --with-tiff=ifavailable --with-gnutls=ifavailable
